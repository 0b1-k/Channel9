...
namespace AdafruitNetduinoLogger {
    ...
    public class Program {
        ...
        public static void LogTemperature(object obj) {
            ...
            var time = AddZeroPrefix(now.Hour) + ":" + AddZeroPrefix(now.Minute) + ":" + AddZeroPrefix(now.Second) + ":" + AddZeroPrefix(now.Millisecond);
                ...
                if (CardDetect.Read() == false) {
                    ...
                    if (File.Exists(filename) == false) {
                        using (var tempLogFile = new StreamWriter(filename, true)) {
                            tempLogFile.WriteLine("date,time,celsius,farenheit");
                        }
                    }
                    using (var tempLogFile = new StreamWriter(filename, true)) {
                        if (Buffer.Count != 0) {
